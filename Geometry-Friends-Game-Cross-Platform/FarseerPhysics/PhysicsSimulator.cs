...
namespace FarseerGames.FarseerPhysics {
    public class PhysicsSimulator {
        ...
        public PhysicsSimulator Clone()
        {
            PhysicsSimulator clone = new PhysicsSimulator(gravity);

            //copy all the properties of this simulator
            clone.iterations = iterations;
            clone.allowedPenetration = allowedPenetration;
            clone.biasFactor = biasFactor;
            clone.maxContactsToDetect = maxContactsToDetect;
            clone.maxContactsToResolve = maxContactsToResolve;
            clone.frictionType = frictionType;

            //we must keep track of body clones with ids so that we can make the proper bindings
            Dictionary<int, Body> sourceToTargetBodyIdsMapping = new Dictionary<int, Body>();
            ...
            {
                tmpBody = b.Clone();
                sourceToTargetBodyIdsMapping[b.Id] = tmpBody;
                clone.bodyList.Add(tmpBody);                
            }
            //being added
            foreach (Body b in bodyAddList)
            {
                tmpBody = b.Clone();
                sourceToTargetBodyIdsMapping[b.Id] = tmpBody;
                clone.bodyAddList.Add(tmpBody);
            }
            //being removed
            foreach (Body b in bodyRemoveList)
            {
                tmpBody = b.Clone();
                sourceToTargetBodyIdsMapping[b.Id] = tmpBody;
                clone.bodyRemoveList.Add(tmpBody);
            }

            //clone GEOMETRY
            Geom tmpGeo;
            //current
            foreach (Geom g in geomList)
            {
                tmpGeo = g.Clone(sourceToTargetBodyIdsMapping);
                clone.geomList.Add(tmpGeo);
                clone.broadPhaseCollider.Add(tmpGeo);
            }
            //being added
            foreach (Geom g in geomAddList)
            {
                tmpGeo = g.Clone(sourceToTargetBodyIdsMapping);
                clone.geomAddList.Add(tmpGeo);
            }
            //being removed
            foreach (Geom g in geomRemoveList)
            {
                tmpGeo = g.Clone(sourceToTargetBodyIdsMapping);
                clone.geomRemoveList.Add(tmpGeo);
            }
            ...
            {
                tmpCont = c.Clone(sourceToTargetBodyIdsMapping);
                clone.controllerList.Add(tmpCont);
            }
            //being added
            foreach (Controller c in controllerAddList)
            {
                tmpCont = c.Clone(sourceToTargetBodyIdsMapping);
                clone.controllerAddList.Add(tmpCont);
            }
            //being removed
            foreach (Controller c in controllerRemoveList)
            {
                tmpCont = c.Clone(sourceToTargetBodyIdsMapping);
                clone.controllerRemoveList.Add(tmpCont);
            }
            ...
            {
                tmpJoint = j.Clone(sourceToTargetBodyIdsMapping);
                clone.jointList.Add(tmpJoint);
            }
            //being added
            foreach (Joint j in jointAddList)
            {
                tmpJoint = j.Clone(sourceToTargetBodyIdsMapping);
                clone.jointAddList.Add(tmpJoint);
            }
            //being removed
            foreach (Joint j in jointRemoveList)
            {
                tmpJoint = j.Clone(sourceToTargetBodyIdsMapping);
                clone.jointRemoveList.Add(tmpJoint);
            }
