...
namespace FarseerGames.FarseerPhysics.Collisions {
    ...
    public class Geom : IEquatable<Geom>, IIsDisposable {
        ...
        public Geom Clone(Dictionary<int,Body> oldToNewBodyMappings)
        {
            Geom clone = new Geom();
            //from construct clone
            clone.collisonGridCellSize = this.collisonGridCellSize;
            ...
            clone.collisionGroup = this.collisionGroup;
            clone.collisionEnabled = this.collisionEnabled;
            clone.collsionResponseEnabled = this.collsionResponseEnabled;
            clone.collisonGridCellSize = this.collisonGridCellSize;
            clone.offset = this.offset;
            clone.rotationOffset = this.rotationOffset;
            clone.collisionCategories = this.collisionCategories;
            clone.collidesWith = this.collidesWith;
            clone.SetVertices(this.localVertices);
            clone.SetBody(oldToNewBodyMappings[body.Id]);
            //additional for perfect cloning
            ...
            if (this.tag != null)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine("Cloning geometry with non value-type tag. Not cloning the tag property.");
                }
            }
            clone.matrix = this.matrix;
            clone.matrixInverse = this.matrixInverse;
            clone.matrixInverseTemp = this.matrixInverseTemp;
