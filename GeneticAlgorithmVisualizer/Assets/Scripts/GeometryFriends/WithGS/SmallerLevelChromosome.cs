...
{
    public class SmallerLevelChromosome : BinaryChromosomeBase
    {
        public const int MAXCOLLECTIBLES = 5;
        public const int MAXPLATFORMS = 8;
        public const int SIZEOFSHORT = 16;
        public const int NUMBEROFBITS = SIZEOFSHORT * (2 + MAXPLATFORMS * 2) +
                                        SIZEOFSHORT * (2 + MAXPLATFORMS * 2) + (MAXPLATFORMS);

        public const int XMax = 1280;

        public const int YMax = 760;
        public const int XYMin = 40;
        // [ RectangleSpawn | CircleSpawn | PlatformArray ]
        // [ Pos            | Pos         | Platform x8   ]
        // Pos = [ x | y ]
        ...
        //   x * (2 + 8*2) + y * (2 + 8*2) + 1 * (8)
        //   16 * (2 + 8*2) + 16 * (2 + 8*2) + (8) = 584 bits
        
        
        public SmallerLevelChromosome() : base(NUMBEROFBITS)
        {
            CreateGenes();
            this.Fitness = null;
        }

        public SmallerLevelChromosome(String s) : base(NUMBEROFBITS)
        {
            CreateGenes();
