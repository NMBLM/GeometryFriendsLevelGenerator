...
namespace FarseerGames.FarseerPhysics.Dynamics {
    ...
    public class Body : IIsDisposable  {
        ...
        //to support proper cloning of the simulator
        private int id;
        ...
        {
            Body clone = new Body();
            //WARNING: inverse mass and moment of inertia are not properly cloned if we use the other constructor to copy thos properties
            clone.mass = this.mass;
            clone.inverseMass = this.inverseMass;
            clone.momentOfInertia = this.momentOfInertia;
            clone.inverseMomentOfInertia = this.inverseMomentOfInertia;
            clone.linearDragCoefficient = this.linearDragCoefficient;
            clone.rotationalDragCoefficient = this.rotationalDragCoefficient;

            clone.position = this.position;
            clone.rotation = this.rotation;
            clone.revolutions = this.revolutions;
            clone.totalRotation = this.totalRotation;
            ...
            clone.previousLinearVelocity = this.previousLinearVelocity;
            clone.previousAngularVelocity = this.previousAngularVelocity;
            clone.linearVelocityBias = this.linearVelocityBias;
            clone.angularVelocityBias = this.angularVelocityBias;
            clone.force = this.force;
            clone.impulse = this.impulse;
            clone.torque = this.torque;
            ...
            if (this.tag != null)
            {
                if (this.tag != null)
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine("Cloning body with non value-type tag. Not cloning the tag property.");
                    }
                }
            }
            clone.ignoreGravity = this.ignoreGravity;
        ...
        private void ApplyDrag() {
            ...
            //commented out for optimization. not necassary without quadratic drag
            ...
            //if (speed > linearDragVelocityThreshhold) {
                ...
                //drag for fast moving objects: may want to implement this in the future
